...
import InitialHistoryDetails from '../InitialHistoryDetails'
import './index.css'
...
class SearchFile extends Component {
  ...
  deleteHistory = id => {
    ...
    if (filterUserData.length === 0) {
      this.setState({HistoryList: filterUserData, isShow: true})
    } else {
      this.setState({HistoryList: filterUserData})
    }
  }
  ...
  render() {
    ...
    const searchResult = HistoryList.filter(eachObj =>
          ...
          <ul>
            {searchResult.length === 0 ? (
              ...
              searchResult.map(eachObj => (
                <InitialHistoryDetails
                  HistoryDetails={eachObj}
